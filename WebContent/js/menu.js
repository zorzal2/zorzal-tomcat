var keepstatic=1 //Especificar si se quiere el menu est?tico o no est?tico (funciona solo en IE4+)
var menucolor="darkslategray" //especificar el color del men?
var submenuwidth=150 //especificar el color del submen?
...
function Menu()
{
	...
	HTMLstr += "<tr>";
	if (n) HTMLstr += "<td width='50px'>&nbsp;";
	HTMLstr += "<!-- MAIN MENU STARTS -->\n";
	HTMLstr += "<!-- MAIN_MENU -->\n";
	HTMLstr += "<!-- MAIN MENU ENDS -->\n";
	if (n) HTMLstr += "</td>";
	HTMLstr += "<td width='35%'>&nbsp;</td></tr>\n";
	HTMLstr += "</table>\n";
	HTMLstr += "\n";
...
function addItem(idItem, text, hint, location, altLocation)
{
	...
	{
		MENUitem += "<td nowrap>\n";
		MENUitem += "<div width='30%' id='"+idItem+"' style='position:relative; font: "+this.menuFont+";'>\n";
		MENUitem += "<a ";
...
function addSubItem(idParent, text, hint, location)
{
	...
	if (HTMLstr.indexOf(Lookup) == -1)
	{
		...
		{
			MENUitem += "\n";
			MENUitem += "<div id='"+idParent+"submenu' onmouseout=operahide() style='position:absolute; visibility: hidden; z-index:0;' width: "+this.subMenuPaneWidth+"; font: "+this.menuFont+"; top: -300;'>\n";
			MENUitem += "<table border='"+this.subMenuPaneBorder+"' bgcolor='"+this.bgColor+"' width="+this.subMenuPaneWidth+">\n";
			MENUitem += "<!-- NEXT ITEM OF SUB MENU "+ idParent +" -->\n";
			MENUitem += "</table>\n";
			MENUitem += "</div>\n";
	...
	if (ie||ns6) {
		if (location == '#'){		
			MENUitem = "<tr><td class=clsMenuItemIE>"+text+"<br></td></tr>\n";
		}else{			
...
{
	var totalOffset = - document.body.scrollLeft;
