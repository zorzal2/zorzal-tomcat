...
public class RubroInsumoParser extends BaseRubroParser {
	...
	protected Collection<Field> getCampos() {
		...
		ret.add(new Field(Labels.ANR.itemInsumos.DETALLE, true));
		ret.add(new Field(Labels.ANR.itemInsumos.UNIDAD_DE_MEDIDA, false));
		ret.add(new Field(Labels.ANR.itemInsumos.CANTIDAD, false));
		ret.add(new Field(Labels.ANR.itemInsumos.COSTO_UNITARIO, false));
		ret.add(new Field(Labels.ANR.itemInsumos.COSTO_TOTAL, true));
		ret.add(new Field(Labels.ANR.itemInsumos.PARTE, false));
		ret.add(new Field(Labels.ANR.itemInsumos.CONTRAPARTE, false));
		return ret;
	}
	...
	protected PresupuestoItemBean crearItem(Map<Label, Cell> map) {
		...
		Cell detalle = map.get(Labels.ANR.itemInsumos.DETALLE);
		item.setNombre(detalle.getContents());
		
		Cell unidadDeMedida = map.get(Labels.ANR.itemInsumos.UNIDAD_DE_MEDIDA);
		item.setUnidadMedida(textOrNull(unidadDeMedida));

		Cell cantidad = map.get(Labels.ANR.itemInsumos.CANTIDAD);
		item.setCantidad(numberOrNull(cantidad));
		
		Cell costoUnitario = map.get(Labels.ANR.itemInsumos.COSTO_UNITARIO);
		item.setCostoUnitario(numberOrNull(costoUnitario));

		Cell costoTotal = map.get(Labels.ANR.itemInsumos.COSTO_TOTAL);
		item.setTotal(((NumberCell)costoTotal).getValue());
		
		Cell parte = map.get(Labels.ANR.itemInsumos.PARTE);
		item.setParte(numberOrNull(parte));

		Cell contraparte = map.get(Labels.ANR.itemInsumos.CONTRAPARTE);
