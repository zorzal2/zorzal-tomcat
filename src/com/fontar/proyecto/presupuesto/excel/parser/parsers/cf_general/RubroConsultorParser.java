...
public class RubroConsultorParser extends BaseRubroParser {
	...
	protected Collection<Field> getCampos() {
		...
		ret.add(new Field(Labels.CFGeneral.itemConsultores.DESCRIPCION, true));
		ret.add(new Field(Labels.CFGeneral.itemConsultores.COSTO_MENSUAL, false));
		ret.add(new Field(Labels.CFGeneral.itemConsultores.PARTICIPACION, false));
		ret.add(new Field(Labels.CFGeneral.itemConsultores.COSTO_TOTAL, true));
		return ret;
	}
	...
	protected PresupuestoItemBean crearItem(Map<Label, Cell> map) {
		...
		Cell descripcion = map.get(Labels.CFGeneral.itemConsultores.DESCRIPCION);
		item.setNombre(descripcion.getContents());

		Cell costoMensual = map.get(Labels.CFGeneral.itemConsultores.COSTO_MENSUAL);
		item.setCostoMensual(numberOrNull(costoMensual));

		Cell participacion = map.get(Labels.CFGeneral.itemConsultores.PARTICIPACION);
		item.setParticipacion(numberOrNull(participacion));

		Cell costoTotal = map.get(Labels.CFGeneral.itemConsultores.COSTO_TOTAL);
	...
	protected PresupuestoItemBean checkItem(PresupuestoItemBean item) throws ValidationException {
		...
		obligatorios(picItem.getCostoMensual(), picItem.getParticipacion());
