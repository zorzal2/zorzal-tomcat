...
public class RubroRecursoHumanoPropioParser extends BaseRubroParser {
	...
	protected Collection<Field> getCampos() {
		...
		ret.add(new Field(Labels.ARAI.itemRecursosHumanosPropios.NOMBRE, true));
		ret.add(new Field(Labels.ARAI.itemRecursosHumanosPropios.PROFESION, false));
		ret.add(new Field(Labels.ARAI.itemRecursosHumanosPropios.ID_TRIBUTARIA, false));
		ret.add(new Field(Labels.ARAI.itemRecursosHumanosPropios.FUNCION, false));
		ret.add(new Field(Labels.ARAI.itemRecursosHumanosPropios.COSTO_MENSUAL, false));
		ret.add(new Field(Labels.ARAI.itemRecursosHumanosPropios.PARTICIPACION, false));
		ret.add(new Field(Labels.ARAI.itemRecursosHumanosPropios.DEDICACION, false));
		ret.add(new Field(Labels.ARAI.itemRecursosHumanosPropios.COSTO_TOTAL, true));
		return ret;
	}
	...
	protected PresupuestoItemBean crearItem(Map<Label, Cell> map) {
		...
		Cell nombre = map.get(Labels.ARAI.itemRecursosHumanosPropios.NOMBRE);
		item.setNombre(textOrNull(nombre));
		
		Cell profesion = map.get(Labels.ARAI.itemRecursosHumanosPropios.PROFESION);
		item.setProfesion(textOrNull(profesion));
		
		Cell idTributaria = map.get(Labels.ARAI.itemRecursosHumanosPropios.ID_TRIBUTARIA);
		item.setIdentificacionTributaria(textOrNull(idTributaria));

		Cell funcion = map.get(Labels.ARAI.itemRecursosHumanosPropios.FUNCION);
		item.setFuncion(textOrNull(funcion));
		
		Cell costoMensual = map.get(Labels.ARAI.itemRecursosHumanosPropios.COSTO_MENSUAL);
		item.setCostoMensual(numberOrNull(costoMensual));

		Cell dedicacion = map.get(Labels.ARAI.itemRecursosHumanosPropios.DEDICACION);
		item.setDedicacion(numberOrNull(dedicacion));
		
		Cell participacion = map.get(Labels.ARAI.itemRecursosHumanosPropios.PARTICIPACION);
		item.setParticipacion(numberOrNull(participacion));

		Cell costoTotal = map.get(Labels.ARAI.itemRecursosHumanosPropios.COSTO_TOTAL);
